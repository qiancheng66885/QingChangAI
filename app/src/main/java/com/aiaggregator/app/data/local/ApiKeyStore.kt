...
class ApiKeyStore(context: Context) {
    ...
    fun saveAllPlatforms(platforms: List<ApiConfig>) {
        prefs.edit().putString(StorageConstants.KEY_PLATFORMS, json.encodeToString(platforms)).apply()
    }

    fun saveConfigSnapshot(platforms: List<ApiConfig>, models: List<ModelConfig>) {
        prefs.edit()
    ...
    fun deletePlatform(id: String) {
        ...
        saveConfigSnapshot(platforms, models)
    }
    ...
    fun getActiveModel(): ModelConfig? = loadModels().find {
        it.isDefault && it.category != ModelCategory.IMAGE
    }
}

...
fun DataScreen() {
    ...
    var deletingHistory by remember { mutableStateOf(false) }
    val busy = clearingCache || deletingHistory
    ...
    Column(
        ...
        ElevatedCard(
            ...
            Column(Modifier.padding(20.dp)) {
                ...
                FilledTonalButton(
                    onClick = { if (!busy) showCacheConfirm = true },
                    enabled = !busy,
        ...
        ElevatedCard(
            ...
            Column(Modifier.padding(20.dp)) {
                Row(verticalAlignment = Alignment.CenterVertically) {
                    Icon(Icons.Filled.DeleteForever, null, Modifier.size(24.dp), tint = MaterialTheme.colorScheme.primary)
                    Spacer(Modifier.width(12.dp))
                    Column {
        ...
        if (showCacheConfirm) {
            AlertDialog(
                onDismissRequest = { if (!clearingCache) showCacheConfirm = false },
                icon = { Icon(Icons.Filled.Storage, null, tint = MaterialTheme.colorScheme.primary) },
                title = { Text("确认清除缓存") },
                text = { Text(if (clearingCache) "正在清除缓存，请稍候。" else "将清除应用临时文件和图片缓存。") },
                confirmButton = {
                    TextButton(onClick = {
                    ...
                    }, enabled = !clearingCache) { Text(if (clearingCache) "清除中..." else "确认清除") }
                },
                dismissButton = {
                    TextButton(onClick = { if (!clearingCache) showCacheConfirm = false }, enabled = !clearingCache) { Text("取消") }
                },
                containerColor = MaterialTheme.colorScheme.surface,
                shape = RoundedCornerShape(20.dp)
            )
        ...
        if (showConfirm && selected != null) {
            AlertDialog(
                onDismissRequest = { if (!deletingHistory) { showConfirm = false; selected = null } },
                icon = { Icon(Icons.Filled.DeleteForever, null, tint = MaterialTheme.colorScheme.primary) },
                title = { Text("确认删除") },
                text = {
                    Text(
                        if (deletingHistory) {
                ...
                confirmButton = {
                    TextButton(onClick = {
                        if (deletingHistory) return@TextButton
                        val target = selected ?: return@TextButton
                        deletingHistory = true
                        scope.launch {
                            try {
                                val deleted = withContext(Dispatchers.IO) {
                                    if (target.timestamp > 0L) {
                                        InMemoryStore.clearNewerThan(target.timestamp)
                                    } else {
                    ...
                    }, enabled = !deletingHistory) { Text(if (deletingHistory) "删除中..." else "确认删除", color = MaterialTheme.colorScheme.error) }
                },
                dismissButton = {
                    TextButton(onClick = { if (!deletingHistory) { showConfirm = false; selected = null } }, enabled = !deletingHistory) { Text("取消") }
                },
                containerColor = MaterialTheme.colorScheme.surface,
                shape = RoundedCornerShape(20.dp)
            )
